...
load("~/climatology_results.RData")
# directories 
train_data <- list(
)
...
for (dataset in gridded_data){
  ...
  if (dataset == "daymet_file"){
    climatology_output <- ifelse(mask == 1, final_climatology$daymet_file, NA)
  }
  ...
  df_outlier_lat_lon <- data.frame(
    ...
    row = outlier_indices[,1],
    col = outlier_indices[,2]
  )
...
# find common and unique outlier indices 
